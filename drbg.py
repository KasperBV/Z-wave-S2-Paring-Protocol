...
class CTRDRBG (DRBG):
    ...
    def _generate(self, count, data=None):
        ...
        while len(temp) < count:
            ...
            temp += self.cipher.new(self.__prepare_key(K), Crypto.Cipher.AES.MODE_ECB).encrypt(V)
    ...
    def __update(self, provided_data, Key, V):
        temp = b''
        cipher = self.cipher.new(self.__prepare_key(Key), Crypto.Cipher.AES.MODE_ECB)
